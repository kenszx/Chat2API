...
import { extractImageUrls, downloadImage } from '../utils/imageUploader'
...
export class QwenAdapter {
  ...
  /**
   * Upload image to Qwen's file service
   */
  private async uploadImage(imageUrl: string): Promise<{ file_url: string } | null> {
    try {
      ...
      const response = await axios.post(
        `${QWEN_CHAT_SIDE_API_BASE}/api/v2/file/upload`,
        formData,
        {
          headers: {
            ...formData.getHeaders(),
      ...
      if (response.status === 200 && response.data?.data?.fileUrl) {
        console.log('[Qwen] Image uploaded:', response.data.data.fileUrl)
        return { file_url: response.data.data.fileUrl }
      }

      // Fallback: try alternative response format
      if (response.status === 200 && response.data?.file_url) {
        console.log('[Qwen] Image uploaded:', response.data.file_url)
        return { file_url: response.data.file_url }
      }

      console.warn('[Qwen] Image upload failed:', response.status, JSON.stringify(response.data))
      return null
    } catch (error) {
      console.error('[Qwen] Image upload error:', error instanceof Error ? error.message : 'Unknown error')
      return null
    }
  }
  ...
  async chatCompletion(request: ChatCompletionRequest): Promise<{
    ...
    // Track if any user message contains images
    const imageUrls = extractImageUrls(request.messages)
    const uploadedImages: Array<{ file_url: string; alt_text?: string }> = []
    ...
    if (imageUrls.length > 0) {
      console.log('[Qwen] Found', imageUrls.length, 'image(s) to upload')
      for (const imgUrl of imageUrls) {
        try {
          const result = await this.uploadImage(imgUrl)
          if (result) {
            uploadedImages.push({ file_url: result.file_url })
          }
        } catch (error) {
          console.error('[Qwen] Image upload failed:', error)
        }
      }
    }
    ...
    // Build messages array: image messages first, then text message
    const qwenMessages: any[] = []

    // Add uploaded image messages
    if (uploadedImages.length > 0) {
      for (const img of uploadedImages) {
        qwenMessages.push({
          content: img.file_url,
          mime_type: 'image/png',
        })
      }
    }
