...
export class KimiAdapter {
  ...
  /**
   * Upload image to Kimi's file service
   */
  private async uploadImage(imageUrl: string): Promise<string | null> {
    try {
      ...
      if (response.status === 200) {
        const fileUrl = response.data?.url || response.data?.file_url || response.data?.data?.url
        if (fileUrl) {
          console.log('[Kimi] Image uploaded:', fileUrl)
          return fileUrl
        }
      }

      console.warn('[Kimi] Image upload failed:', response.status, JSON.stringify(response.data).slice(0, 200))
      return null
    } catch (error) {
      console.error('[Kimi] Image upload error:', error instanceof Error ? error.message : 'Unknown error')
      return null
    }
  }
  ...
  async chatCompletion(request: ChatCompletionRequest): Promise<{ response: AxiosResponse; conversationId: string }> {
    ...
    // Upload images and inject URLs into message content
    const imageUrls = extractImageUrls(messages)
    if (imageUrls.length > 0) {
      console.log('[Kimi] Found', imageUrls.length, 'image(s) to upload')
      for (const imgUrl of imageUrls) {
        try {
          const uploadedUrl = await this.uploadImage(imgUrl)
          if (uploadedUrl) {
            // Find and replace the user message containing this image
            for (let i = 0; i < messages.length; i++) {
              const msg = messages[i]
              if (!Array.isArray(msg.content)) continue
              const hasImage = msg.content.some((p: any) =>
                p.type === 'image_url' && p.image_url?.url === imgUrl
              )
              if (hasImage) {
                const textParts = msg.content
                  .filter((p: any) => p.type === 'text')
                  .map((p: any) => p.text || '')
                textParts.push(`![image](${uploadedUrl})`)
                messages[i] = { ...msg, content: textParts.join('\n') }
                break
              }
            }
          }
        } catch (error) {
          console.error('[Kimi] Image processing failed:', error)
        }
      }
    }
