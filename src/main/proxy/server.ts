  ...
  private setupMiddleware(): void {
    ...
    this.app.use(async (ctx, next) => {
      ...
      if (config.enableApiKey && config.apiKeys && config.apiKeys.length > 0) {
        // Support multiple auth methods: Authorization Bearer, x-api-key (Anthropic style), query param
        const authHeader = ctx.get('Authorization') || ''
        const xApiKey = ctx.get('x-api-key') || ctx.get('X-API-Key')
        const providedKey = authHeader.startsWith('Bearer ')
          ? authHeader.slice(7)
          : xApiKey || (ctx.query.api_key as string)
